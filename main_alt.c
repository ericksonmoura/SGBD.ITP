...
void tituloEidioma(){
    ...
    SetConsoleTitle("Projeto ITP - SGBD");
}
...
char tipoVariavel(*canal_metadados){

        int x, roll = 0;
        char tipo[30]; // preciso alocar dinamicamente
        char tipo_var[4];
        char nome[30];
        system("cls");
        ...
        strcat(tipo,nome);
        strcat(tipo,"$");
        strcat(tipo,tipo_var);
        strcat(tipo,";\n");

        fprintf(canal_metadados,tipo);




    //fprintf(metaDados,"%s","}\n");
    fclose(canal_metadados);
